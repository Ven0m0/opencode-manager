...
const createMockSessionStateServiceWithPlanning = (
    ...
    session: {
      ...
      messages: async () => ({
        data: [
          {
            info: { role: "assistant" },
            parts: [{ type: "text", text: "Compaction summary text" }],
          },
        ],
      }),
...
describe("SessionHooks", () => {
        ...
        session: {
          messages: async () => ({
            data: [
              {
                info: { role: "assistant" },
                parts: [{ type: "text", text: "Compaction summary content" }],
              },
            ],
          }),
  ...
  test("session.compacted includes planning state in subtask prompt when available", async () => {
    ...
    const planningState: PlanningState = {
      ...
      phases: [
        {
          title: "Update buildSubtaskPrompt",
          status: "completed",
          notes: "Added planningState parameter",
        },
        {
          title: "Update runPostCompactionFlow",
          status: "in_progress",
          notes: "Fetching from KV store",
        },
        ...
        session: {
          messages: async () => ({
            data: [
              {
                info: { role: "assistant" },
                parts: [{ type: "text", text: "Compaction summary here" }],
              },
            ],
          }),
          create: async () => ({ data: { id: "unused" } }),
  ...
  test("session.compacted with active planning includes resume instruction", async () => {
    ...
    const customMockPluginInput: PluginInput = {
      client: {
        session: {
          messages: async () => ({
            data: [{ info: { role: "assistant" }, parts: [{ type: "text", text: "Summary" }] }],
          }),
          create: async () => ({ data: { id: "unused" } }),
          prompt: async (call: unknown) => {
            promptCall = call;
