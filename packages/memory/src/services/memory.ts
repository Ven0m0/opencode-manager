...
import type { Logger, Memory, MemoryScope, MemoryStats } from "../types";
...
export class MemoryService {
  ...
  setDedupThreshold(threshold: number): void {
    this.dedupThreshold = Math.max(0.05, Math.min(0.4, threshold));
  }
  ...
  async create(input: CreateMemoryInput): Promise<{ id: number; deduplicated: boolean }> {
    ...
    const similar = await this.queries.findSimilar(
      embedding,
      input.projectId,
      this.dedupThreshold,
      1,
