    ...
    try {
      ...
      while (pending > 0 && iterations < EmbeddingSyncService.MAX_ITERATIONS) {
        ...
        for (const memory of memories) {
          ...
          while (!success && attempts < this.maxRetries) {
            ...
            if (!success && attempts < this.maxRetries) {
              this.logger?.log(
                `[embedding-sync] Retrying memory ${memory.id} (attempt ${attempts + 1}/${this.maxRetries})`,
              );
            }
          }
          ...
          if (success) {
            ...
            batchFailed++;
            this.logger?.error(
              `[embedding-sync] Failed to embed memory ${memory.id} after ${this.maxRetries} attempts`,
            );
          }
        }
        ...
        if (batchSuccess === 0 && batchFailed > 0) {
          this.logger?.error(
            "[embedding-sync] No memories embedded in this batch, stopping to prevent infinite loop",
          );
          break;
        }
        ...
        if (pending > 0) {
          this.logger?.log(
            `[embedding-sync] Progress: ${embedded} embedded, ${failed} failed, ${pending} remaining`,
          );
        }
      }
