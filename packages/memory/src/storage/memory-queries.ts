...
export function createMemoryQuery(db: Database, vec: VecService, logger?: Logger) {
  ...
  return {
    ...
    async updateEmbedding(memoryId: number, embedding: number[]): Promise<void> {
      ...
      if (embedding.length > 0) {
        const projectId = db
          ...
          .get(memoryId) as { project_id: string } | undefined;
        if (projectId) {
    ...
    async search(
      ...
      try {
        const vecResults = await vec.search(
          embedding,
          projectId,
          filters?.scope,
          filters?.limit ?? 10,
    ...
    getMemoriesWithoutEmbeddings(projectId?: string, limit: number = 50): Memory[] {
      const embeddingsTableExists = db
    ...
    async deleteByFilePath(projectId: string, filePath: string): Promise<void> {
      if (vec.available) {
        const rows = db
          .prepare("SELECT id FROM memories WHERE project_id = ? AND file_path = ?")
      ...
      db.prepare("DELETE FROM memories WHERE project_id = ? AND file_path = ?").run(
        projectId,
        filePath,
      );
    },
