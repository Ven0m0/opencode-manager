...
export type TableDimensionsResult = { exists: false } | { exists: true; dimensions: number | null };

export function getTableDimensions(db: Database): TableDimensionsResult {
  const row = db
    .prepare("SELECT sql FROM sqlite_master WHERE type='table' AND name='memory_embeddings'")
