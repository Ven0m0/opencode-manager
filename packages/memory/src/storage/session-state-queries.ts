...
export function createSessionStateQueries(db: Database) {
  ...
  const deleteByPrefixStmt = db.prepare(`DELETE FROM session_state WHERE key LIKE ?`);
  ...
  return {
    ...
    searchByProjectAndPrefix(
      ...
      const rows = searchByProjectAndPrefixStmt.all(
        projectId,
        keyPattern,
        searchPattern,
        now,
      ) as SessionStateRowRaw[];
      return rows.map(mapRow);
    },
    ...
    listByProjectAndPrefix(projectId: string, keyPrefix: string): SessionStateRow[] {
      const now = Date.now();
      const keyPattern = `${escapeLikeWildcards(keyPrefix)}%`;
      const rows = listByProjectAndPrefixStmt.all(
        projectId,
        keyPattern,
        now,
      ) as SessionStateRowRaw[];
