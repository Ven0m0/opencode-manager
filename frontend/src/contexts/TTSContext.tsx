...
export function TTSProvider({ children }: TTSProviderProps) {
  ...
  const synthesizeExternal = useCallback(
  ...
  const fetchNextChunk = useCallback(
    async (index: number) => {
      ...
      try {
        const blob = await synthesizeExternal(
  ...
  const playChunk = useCallback(
    async (index: number) => {
      ...
      try {
        ...
        if (!blob) {
          setState("loading");
          const fetched = await synthesizeExternal(
            chunksRef.current[index],
            abortControllerRef.current?.signal,
          );
  ...
  const speakBuiltinWithConfig = useCallback(
    async (text: string, config: TTSConfig): Promise<boolean> => {
  ...
  const speakWithConfig = useCallback(
    async (text: string, config: TTSConfig): Promise<boolean> => {
      if (!config.enabled) {
