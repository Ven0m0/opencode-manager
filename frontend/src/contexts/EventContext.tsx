...
export function EventProvider({ children }: { children: React.ReactNode }) {
  ...
  const [sshHostKeyRequest, setSSHHostKeyRequest] =
    useState<SSHHostKeyRequest | null>(null);

  const respondToSSHHostKey = useCallback(
  ...
  const [permissionsBySession, setPermissionsBySession] =
    useState<PermissionsBySession>({});
  const [questionsBySession, setQuestionsBySession] =
    useState<QuestionsBySession>({});
  ...
  const allPermissions = useMemo(
    () => Object.values(permissionsBySession).flat(),
    [permissionsBySession],
  );
  const allQuestions = useMemo(
    () => Object.values(questionsBySession).flat(),
    [questionsBySession],
  );
  ...
  const getRepoIdForSession = useCallback(
  ...
  const getClient = useCallback(
    (sessionID: string): OpenCodeClient | null => {
      const cache = queryClient.getQueryCache();
      const queries = cache.getAll();
  ...
  const removePermission = useCallback(
    (permissionID: string, sessionID?: string) => {
      removeFromSessionKeyedState(
  ...
  useEffect(() => {
    ...
    if (
      permissionCount > prevPermissionCountRef.current &&
      permissionCount > 0 &&
  ...
  const getQuestionForCallID = useCallback(
    (callID: string, sessionID: string): QuestionRequest | null => {
  ...
  const hasPermissionsForSession = useCallback(
  ...
  const hasQuestionsForSession = useCallback(
    (sessionID: string): boolean => {
    ...
    for (const directory of uniqueDirectories) {
      try {
        ...
        if (import.meta.env.DEV) {
          console.warn(
            `Failed to fetch pending questions for ${directory}:`,
            error,
          );
        }
      }
    ...
    const handleSSEMessage = (data: unknown) => {
      ...
      switch (event.type) {
        case "permission.asked":
          if (
            "permission" in event.properties &&
        ...
        case "question.rejected":
          if (
            ...
            removeQuestion(event.properties.requestID as string, sessionID);
