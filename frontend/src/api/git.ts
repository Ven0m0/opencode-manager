...
import type { CommitDetails, FileDiffResponse, GitCommit, GitStatusResponse } from "@/types/git";
...
export async function fetchReposGitStatus(
  ...
  const data = await fetchWrapper<Record<string, GitStatusResponse>>(
    `${API_BASE_URL}/api/repos/git-status-batch`,
    {
      method: "POST",
...
export function useCommitFileDiff(
  ...
  return useQuery({
    ...
    queryFn: () =>
      repoId && commitHash && path
        ? fetchCommitFileDiff(repoId, commitHash, path)
        : Promise.reject(new Error("Missing params")),
...
export function useCommitDetails(repoId: number | undefined, hash: string | undefined) {
  return useQuery({
    ...
    queryFn: () =>
      repoId && hash
        ? fetchCommitDetails(repoId, hash)
        : Promise.reject(new Error("Missing params")),
    enabled: !!repoId && !!hash,
  });
}
