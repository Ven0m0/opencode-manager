...
import type { ChunkedFileInfo, FileInfo, PatchOperation } from "@/types/files";
...
export function useFile(path: string | undefined) {
  return useQuery({
    ...
    queryFn: () =>
      path
        ? fetchFile(path)
        : Promise.reject(new Error("No file path provided")),
    enabled: !!path,
  });
}
...
export async function fetchFileRange(
  ...
  const response = await fetch(
    `${API_BASE_URL}/api/files/${path}?startLine=${startLine}&endLine=${endLine}`,
  );
...
export async function getIgnoredPaths(
  path: string,
): Promise<{ ignoredPaths: string[] }> {
  const response = await fetch(
    `${API_BASE_URL}/api/files/${path}/ignored-paths`,
  );
