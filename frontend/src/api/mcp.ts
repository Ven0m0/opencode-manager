...
export const mcpApi = {
  ...
  async checkFlowStatus(flowId: string): Promise<McpOAuthFlowStatus> {
    try {
      return await fetchWrapper(
        `${API_BASE_URL}/api/mcp-oauth-proxy/status/${encodeURIComponent(flowId)}`,
      );
    } catch {
  ...
  async completeAuth(name: string, code: string): Promise<McpStatus> {
    return fetchWrapper(
      `${API_BASE_URL}/api/opencode/mcp/${encodeURIComponent(name)}/auth/callback`,
      {
        method: "POST",
        headers: { "Content-Type": "application/json" },
  ...
  async authenticate(name: string): Promise<McpStatus> {
    return fetchWrapper(
      `${API_BASE_URL}/api/opencode/mcp/${encodeURIComponent(name)}/auth/authenticate`,
      {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({}),
  ...
  async connectDirectory(name: string, directory: string): Promise<boolean> {
    return fetchWrapper(
      `${API_BASE_URL}/api/settings/mcp/${encodeURIComponent(name)}/connectdirectory`,
      {
        method: "POST",
        headers: { "Content-Type": "application/json" },
  ...
  async disconnectDirectory(name: string, directory: string): Promise<boolean> {
    return fetchWrapper(
      `${API_BASE_URL}/api/settings/mcp/${encodeURIComponent(name)}/disconnectdirectory`,
      {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ directory }),
  ...
  async authenticateDirectory(name: string, directory: string): Promise<McpStatus> {
    return fetchWrapper(
      `${API_BASE_URL}/api/settings/mcp/${encodeURIComponent(name)}/authdirectedir`,
      {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ directory }),
