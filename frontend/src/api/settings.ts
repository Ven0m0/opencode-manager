...
export const settingsApi = {
  ...
  deleteOpenCodeConfig: async (configName: string, userId = DEFAULT_USER_ID): Promise<boolean> => {
    await fetchWrapper(
      `${API_BASE_URL}/api/settings/opencode-configs/${encodeURIComponent(configName)}`,
      {
        method: "DELETE",
  ...
  restartOpenCodeServer: async (): Promise<{
    success: boolean;
    message: string;
    details?: string;
  }> => {
  ...
  reloadOpenCodeConfig: async (): Promise<{
    success: boolean;
    message: string;
    details?: string;
  }> => {
  ...
  rollbackOpenCodeConfig: async (): Promise<{
    success: boolean;
    message: string;
    configName?: string;
  }> => {
