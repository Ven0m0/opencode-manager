...
import {
  VirtualizedTextView,
  type VirtualizedTextViewHandle,
...
export const FilePreview = memo(function FilePreview({
  ...
  }, [initialLineNumber, shouldVirtualize]);
  ...
  const handleEdit = () => {
    if (shouldVirtualize) {
      setViewMode("edit");
      const event = new CustomEvent("editModeChange", {
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      setViewMode("preview");
      const editEvent = new CustomEvent("editModeChange", {
        detail: { isEditing: false },
      });
      window.dispatchEvent(editEvent);
      const event = new CustomEvent("fileSaved", {
        detail: { path: file.path, content: editContent },
      });
      window.dispatchEvent(event);
      onFileSaved?.();
    } catch (err) {
  ...
  const renderContent = () => {
    ...
    if (isTextFile) {
      ...
      try {
        ...
        return (
          <div
            className={`pb-[200px] text-sm bg-muted text-foreground rounded font-mono ${
  ...
  return (
    <div className="h-full flex flex-col bg-background">
      {!hideHeader && (
        <div
          className={`flex items-start gap-2 px-3 py-2 border-b border-border flex-shrink-0 overflow-hidden ${isMobileModal ? "pt-3" : ""}`}
        >
          ...
          <div className="flex items-center gap-1 flex-shrink-0 mt-1">
            ...
            {showCancelButton && (
              <Button
                ...
                className="border-destructive bg-destructive/10 text-destructive hover:bg-destructive/20 h-7 w-7 p-0"
              >
