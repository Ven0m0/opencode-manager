...
function ClickableJson({
  ...
  if (references.length === 0) {
    return (
      <pre className="bg-accent p-2 rounded text-xs overflow-x-auto whitespace-pre-wrap break-words">
        {jsonString}
      </pre>
    );
  }
...
export function ToolCallPart({
  ...
  return (
    <div
      ref={outputRef}
      className={`border rounded-lg overflow-hidden my-2 transition-all ${getBorderStyle()}`}
    >
      <button
