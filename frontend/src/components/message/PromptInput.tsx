...
export const PromptInput = forwardRef<PromptInputHandle, PromptInputProps>(function PromptInput(
  ...
  const handlePaste = async (event: React.ClipboardEvent<HTMLTextAreaElement>) => {
    ...
    const isIOS =
      /iPad|iPhone|iPod/.test(navigator.userAgent) && !(window as { MSStream?: boolean }).MSStream;
  ...
  const handleKeyDown = (e: KeyboardEvent<HTMLTextAreaElement>) => {
    ...
    if (showMentionSuggestions && mentionItems.length > 0) {
      if (e.key === "ArrowDown") {
        e.preventDefault();
        setSelectedMentionIndex((prev) => (prev < mentionItems.length - 1 ? prev + 1 : prev));
        return;
      }
  ...
  return (
    ...
    >
      ...
      <textarea
        ...
        onDrop={handleDrop}
        placeholder={isBashMode ? "Enter bash command..." : "Send a message..."}
        disabled={disabled}
          ...
          !hasPendingPermissionForSession ? (
            ...
            <button
              ...
              disabled={
                hasPendingPermissionForSession
                  ? false
                  : (!prompt.trim() && imageAttachments.length === 0) || disabled
              }
            ...
            >
              <span className="whitespace-nowrap">
                {hasPendingPermissionForSession ? "View" : hasActiveStream ? "Queue" : "Send"}
              </span>
            </button>
          )}
        </div>
