import { ChevronDown, Mic, MicOff, Upload, X } from "lucide-react";
import {
  forwardRef,
  type KeyboardEvent,
  useEffect,
...
import type { FileInfo, ImageAttachment, MessageWithParts } from "@/api/types";
...
import { type MentionItem, MentionSuggestions } from "./MentionSuggestions";
...
export const PromptInput = forwardRef<PromptInputHandle, PromptInputProps>(
  function PromptInput(
    ...
    const handleSubmit = () => {
      ...
      if (hasActiveStream) {
        const parts = parsePromptToParts(
          prompt,
          attachedFiles,
          imageAttachments,
        );
      ...
      if (command.template) {
        ...
        if (commandMatch) {
          ...
          setTimeout(() => {
            if (textareaRef.current) {
              const newCursorPos =
                beforeCommand.length + command.name.length + 2;
              textareaRef.current.focus();
    ...
    useEffect(() => {
      ...
      if (
        !isRecording &&
        textToUse &&
        textToUse !== "Processing..." &&
        textToUse !== "Recording..."
      ) {
        const trimmedTranscript = textToUse.trim();
        if (
          trimmedTranscript &&
          trimmedTranscript !== lastAddedTranscriptRef.current
        ) {
          if (
            prompt === "" ||
            prompt === "Processing..." ||
    ...
    const handleKeyDown = (e: KeyboardEvent<HTMLTextAreaElement>) => {
      ...
      if (
        e.key === "Enter" &&
        (e.metaKey || e.ctrlKey || (isMobile && !e.shiftKey))
      ) {
        e.preventDefault();
        if (isMobile) {
          textareaRef.current?.blur();
    ...
    const hasIncompleteMessages = lastAssistantMessage
      ? isMessageIncomplete(lastAssistantMessage)
      : false;
    ...
    const { setShowDialog, hasForSession: hasPermissionsForSession } =
      usePermissions();
    const hasPendingPermissionForSession = hasPermissionsForSession(sessionID);
    const { hasVariants, currentVariant, cycleVariant } = useVariants(
      opcodeUrl,
      directory,
    );
    ...
    return (
      <div
        className={`relative backdrop-blur-md bg-background opacity-95 border border-border dark:border-white/30 rounded-xl p-2 md:p-3 mb-4 md:mb-1 w-full transition-all ${hasPendingPermissionForSession ? "border-orange-500/50 ring-1 ring-orange-500/30" : ""}`}
      >
        ...
        <textarea
          ...
          onDrop={handleDrop}
          placeholder={
            isBashMode ? "Enter bash command..." : "Send a message..."
          }
          disabled={disabled}
