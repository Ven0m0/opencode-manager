...
const variantConfig: Record<
  ...
  { icon: ReactNode; label: string; colors: string }
> = {
  clear: {
    icon: <X className="w-5 h-5" />,
    label: "Clear",
    colors:
      "bg-gradient-to-br from-red-600 to-red-700 hover:from-red-500 hover:to-red-600 border-2 border-red-500/60 hover:border-red-400 shadow-lg shadow-red-500/30 hover:shadow-red-500/50 ring-2 ring-red-500/20 hover:ring-red-500/40 text-destructive-foreground",
  },
  "stop-audio": {
  ...
  custom: {
    ...
    label: "",
    colors:
      "bg-muted hover:bg-muted-foreground/20 text-muted-foreground border-border",
  },
};
...
export const FloatingActionButton = memo(function FloatingActionButton({
  ...
  return (
    ...
    >
      {loading ? <Loader2 className="w-5 h-5 animate-spin" /> : displayIcon}
      {displayLabel && (
