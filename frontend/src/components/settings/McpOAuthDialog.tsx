    ...
    <Dialog open={open} onOpenChange={handleOpenChange}>
      <DialogContent className="w-[calc(100vw-2rem)] max-w-md h-auto">
        <DialogHeader>
          ...
          <DialogDescription>Authenticate to use this MCP server {scopes}</DialogDescription>
        </DialogHeader>
        ...
        <div className="space-y-4 py-2 min-w-0">
          ...
          {step === "ready" && authUrl && (
            <div className="space-y-3">
              <p className="text-sm text-muted-foreground">
                Click below to open the authorization page. After you approve access, this dialog
                will update automatically.
              </p>
              <Button onClick={handleOpenAuthPage} className="w-full">
          ...
          {step === "waiting" && (
            <div className="space-y-4">
              ...
              <p className="text-xs text-center text-muted-foreground">
                Complete the authorization in the popup window. This dialog will update
                automatically.
              </p>
          ...
          {step === "popup_closed" && (
            <div className="space-y-4">
              ...
              <p className="text-sm text-muted-foreground">
                If authorization was completed, this will update shortly. Otherwise, paste the{" "}
                <code className="text-xs bg-muted px-1 py-0.5 rounded">code</code> parameter from
                the redirect URL below.
              </p>
              ...
              {authUrl && (
                <Button variant="outline" size="sm" className="w-full" onClick={handleOpenAuthPage}>
                  <ExternalLink className="h-4 w-4 mr-2" />
                  Try Again
                </Button>
              )}
      ...
      {showManualEntry && (
        <div className="space-y-2 mt-3">
          <p className="text-xs text-muted-foreground">
            After authorizing, copy the <code className="bg-muted px-1 py-0.5 rounded">code</code>{" "}
            parameter from the redirect URL in your browser.
          </p>
          <Label htmlFor="auth-code" className="text-xs">
            Authorization Code
          </Label>
          <div className="flex gap-2">
            <Input
              ...
              className="text-sm"
            />
            <Button onClick={onSubmit} disabled={loading || !authCode.trim()} size="sm">
              {loading ? <Loader2 className="h-4 w-4 animate-spin" /> : "Submit"}
            </Button>
          </div>
        </div>
