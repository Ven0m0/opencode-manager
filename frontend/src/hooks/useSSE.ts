    ...
    (event: SSEEvent) => {
      switch (event.type) {
        ...
        case "messagev2.part.updated": {
          ...
          const currentData = queryClient.getQueryData<MessageListResponse>([
            "opencode",
            "messages",
            opcodeUrl,
            sessionID,
            ...
            const updatedParts = msg.parts.map((part) => {
              ...
              return {
                ...
                state: {
                  ...
                  output:
                    part.state.status === "running"
                      ? "[Session ended - output not captured]"
                      : "[Tool was pending when session ended]",
                  title:
                    part.state.status === "running"
