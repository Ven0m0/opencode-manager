...
>;
const deleteFileOrFolder = fileService.deleteFileOrFolder as MockedFunction<
  typeof fileService.deleteFileOrFolder
>;
const renameOrMoveFile = fileService.renameOrMoveFile as MockedFunction<
  typeof fileService.renameOrMoveFile
>;
const applyFilePatches = fileService.applyFilePatches as MockedFunction<
...
>;
const getArchiveStream = archiveService.getArchiveStream as MockedFunction<
  typeof archiveService.getArchiveStream
>;
...
describe("File Routes", () => {
  ...
  describe("GET /* with ?startLine&endLine - File Range", () => {
    ...
    it("should return 400 for invalid startLine", async () => {
      const response = await app.request(
        "/api/files/test-repo/test.ts?startLine=invalid&endLine=10",
      );
      const body = (await response.json()) as { error: string };
    ...
    it("should return 400 for invalid endLine", async () => {
      const response = await app.request(
        "/api/files/test-repo/test.ts?startLine=0&endLine=invalid",
      );
      const body = (await response.json()) as { error: string };
