...
import {
  consumeMcpOAuthFlow,
  deleteMcpOAuthFlow,
  getMcpOAuthFlowResult,
  markMcpOAuthFlowCompleted,
  markMcpOAuthFlowFailed,
  storeMcpOAuthFlow,
} from "../services/mcp-oauth-state";
...
// biome-ignore lint/suspicious/noExplicitAny: middleware type varies by auth config
export function createMcpOauthProxyRoutes(requireAuth?: any) {
  ...
  app.post("/start", async (c) => {
    try {
      ...
      const { serverName, serverUrl, scope, clientId, clientSecret, directory } =
      ...
      if (!resolvedClientId) {
        if (!metadata.registration_endpoint) {
          return c.json(
            {
              error: "Server does not support dynamic client registration and no clientId provided",
            },
            400,
          );
        }
  ...
  app.get("/callback", async (c) => {
    ...
    if (!state) {
      return c.html(
        renderPage("Missing State", "No state parameter. Please try again.", false),
        400,
      );
    }
    ...
    if (!code) {
      ...
      deleteMcpOAuthFlow(state);
      return c.html(
        renderPage("Missing Code", "No authorization code. Please try again.", false),
        400,
      );
    }
    ...
    if (!flow) {
      return c.html(
        renderPage("Session Expired", "Authorization session expired. Please try again.", false),
        400,
      );
    }
    ...
    try {
      ...
      if (!tokenResponse.ok) {
        ...
        logger.error(
          `Token exchange failed for ${flow.serverName}: ${tokenResponse.status} ${errText}`,
        );
      ...
      try {
        ...
        logger.warn(
          `Failed to trigger reconnect for ${flow.serverName}, may need manual reconnect`,
        );
      }

      return c.html(
        renderPage("Authentication Successful", "You can close this window now.", true),
      );
    } catch (err) {
      logger.error("MCP OAuth callback failed:", err);
      markMcpOAuthFlowFailed(state, "Unexpected error during token exchange");
      return c.html(
        renderPage("Unexpected Error", "An error occurred. Please try again.", false),
        500,
      );
    }
  });
