...
export function createMemoryRoutes(db: Database): Hono {
  ...
  app.get("/project-summary", async (c) => {
    ...
    try {
      ...
      if (!repo) {
        return c.json(
          { projectId: null, stats: { total: 0, byScope: {} }, error: "Repository not found" },
          404,
        );
      }
    ...
    try {
      ...
      try {
        ...
        return c.json({
          success: true,
          message:
            "Cleared embeddings. Server restart required to regenerate embeddings with new model.",
