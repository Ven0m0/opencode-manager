...
export function createSettingsRoutes(db: Database) {
  ...
  app.patch("/", async (c) => {
    try {
      ...
      const credentialsChanged =
        validated.preferences.gitCredentials !== undefined &&
        JSON.stringify(currentSettings.preferences.gitCredentials || []) !==
          JSON.stringify(validated.preferences.gitCredentials);

      const identityChanged =
        validated.preferences.gitIdentity !== undefined &&
        JSON.stringify(currentSettings.preferences.gitIdentity || {}) !==
          JSON.stringify(validated.preferences.gitIdentity);
  ...
  app.post("/opencode-install-version", async (c) => {
    ...
    try {
      ...
      const installMethod = getOpenCodeInstallMethod();
      logger.info(
        `Running opencode upgrade ${versionArg} --method ${installMethod} with 90s timeout...`,
      );
  ...
  app.post("/custom-commands", async (c) => {
    try {
      ...
      const settings = settingsService.getSettings(userId);
      const existingCommand = settings.preferences.customCommands.find(
        (cmd) => cmd.name === validated.name,
      );
      if (existingCommand) {
  ...
  app.put("/custom-commands/:name", async (c) => {
    try {
      ...
      const settings = settingsService.getSettings(userId);
      const commandIndex = settings.preferences.customCommands.findIndex(
  ...
  app.delete("/custom-commands/:name", async (c) => {
    try {
      ...
      const settings = settingsService.getSettings(userId);
      const commandExists = settings.preferences.customCommands.some(
        (cmd) => cmd.name === commandName,
      );
      if (!commandExists) {
        return c.json({ error: "Command not found" }, 404);
      ...
      const updatedCommands = settings.preferences.customCommands.filter(
        (cmd) => cmd.name !== commandName,
      );
      settingsService.updateSettings(
        {
      ...
      try {
        ...
        if (timedOut) {
          ...
          return c.json({
            success: false,
            message:
              "Connection timed out. This may indicate a network issue or an incorrect host.",
          });
        }
        ...
        if (
          ...
          return c.json({
            success: false,
            message:
              "Permission denied. The SSH key may not be authorized on this host, or the passphrase is incorrect.",
          });
        }

        if (
          outputStr.includes("Could not resolve hostname") ||
          outputStr.includes("Name or service not known")
        ) {
          return c.json({
            success: false,
            message:
              "Could not resolve hostname. Please check that the host is correct and accessible.",
          });
        }

        if (
          outputStr.includes("Connection refused") ||
          outputStr.includes("Connection timed out")
        ) {
          return c.json({
            success: false,
            message:
              "Connection refused or timed out. The host may be down or not accepting SSH connections.",
          });
        }

        const authenticated =
          outputStr.includes("successfully authenticated") ||
