  ...
  app.get("*", async (c) => {
    ...
    if (path.endsWith("/download-zip")) {
      ...
      try {
        ...
        const includePaths = includePathsParam
          ? includePathsParam.split(",").map((p: string) => p.trim())
          : undefined;
        ...
        const archivePath = await archiveService.createDirectoryArchive(
          userPath,
          undefined,
          options,
        );
        ...
        return c.json(
          { error: getErrorMessage(error) || "Failed to create archive" },
          getStatusCode(error) as ContentfulStatusCode,
        );
      }
    }
    ...
    if (path.endsWith("/ignored-paths")) {
      ...
      try {
        ...
        return c.json(
          { error: getErrorMessage(error) || "Failed to get ignored paths" },
          getStatusCode(error) as ContentfulStatusCode,
        );
      }
    }
