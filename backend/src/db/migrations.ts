...
export function runMigrations(db: Database): void {
  try {
    ...
    const repoUrlColumn = tableInfo.find(
      (col: ColumnInfo) => col.name === "repo_url",
    );
    if (repoUrlColumn && repoUrlColumn.notnull === 1) {
      logger.info(
        "Migrating repos table to allow nullable repo_url for local repos",
      );
      ...
      try {
        ...
        db.run(
          `INSERT INTO repos_new (${columnsStr}) SELECT ${columnsStr} FROM repos`,
        );
        ...
        db.run("COMMIT");
        logger.info(
          "Successfully migrated repos table to allow nullable repo_url",
        );
    ...
    try {
      ...
      logger.debug(
        "Local path index already exists or could not be created",
        error,
      );
    }
    ...
    const requiredColumns = [
      {
        name: "default_branch",
        sql: "ALTER TABLE repos ADD COLUMN default_branch TEXT",
      },
      {
        name: "clone_status",
        sql: 'ALTER TABLE repos ADD COLUMN clone_status TEXT NOT NULL DEFAULT "cloning"',
      },
      {
        name: "cloned_at",
        sql: "ALTER TABLE repos ADD COLUMN cloned_at INTEGER NOT NULL DEFAULT 0",
      },
      {
        name: "last_pulled",
        sql: "ALTER TABLE repos ADD COLUMN last_pulled INTEGER",
      },
      {
        name: "opencode_config_name",
        sql: "ALTER TABLE repos ADD COLUMN opencode_config_name TEXT",
      },
      {
        name: "is_worktree",
        sql: "ALTER TABLE repos ADD COLUMN is_worktree BOOLEAN DEFAULT FALSE",
      },
      {
        name: "is_local",
        sql: "ALTER TABLE repos ADD COLUMN is_local BOOLEAN DEFAULT FALSE",
      },
    ];
    ...
    try {
      const repos = db
        .prepare(
          "SELECT id, local_path FROM repos WHERE local_path LIKE 'repos/%'",
        )
        .all() as Array<{
        id: number;
        local_path: string;
      ...
      if (repos.length > 0) {
        logger.info(
          `Migrating ${repos.length} repos to remove 'repos/' prefix from local_path`,
        );
        ...
        for (const repo of repos) {
          ...
          logger.info(
            `Updated repo ${repo.id}: ${repo.local_path} -> ${newPath}`,
          );
        }
      }
    } catch (error) {
...
function migrateGitTokenToCredentials(db: Database): void {
  try {
    const rows = db
      .prepare("SELECT user_id, preferences FROM user_preferences")
      .all() as Array<{
      user_id: string;
      preferences: string;
    }>;
    ...
    for (const row of rows) {
      try {
        ...
        const existingCredentials = parsed.gitCredentials as
          | Array<unknown>
          | undefined;
        ...
        logger.info(
          `Migrated gitToken to gitCredentials for user: ${row.user_id}`,
        );
      } catch (parseError) {
        logger.error(
          `Failed to parse preferences for user ${row.user_id}:`,
          parseError,
        );
      }
    }
