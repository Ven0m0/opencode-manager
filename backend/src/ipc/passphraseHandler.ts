...
export class PassphraseHandler implements IPCHandler {
  private resolveMap = new Map<
    string,
    { resolve: (passphrase: string) => void; reject: (error: Error) => void }
  >();
  ...
  constructor(ipcServer: IPCServer | undefined) {
    if (ipcServer) {
      ...
      logger.warn(
        "PassphraseHandler: No IPC server provided, passphrase prompts will fail",
      );
    }
  }
  ...
  cleanup(): void {
    this.resolveMap.forEach(({ reject }) =>
      reject(new Error("Passphrase handler closed")),
    );
