...
import { normalizeHostPort, parseSSHHost } from "../utils/ssh-key-manager";
...
export class SSHHostKeyHandler implements IPCHandler {
  ...
  async verifyHostKeyBeforeOperation(repoUrl: string): Promise<boolean> {
    ...
    try {
      ...
      logger.warn(
        `Failed to fetch host key for ${hostPort}, rejecting connection:`,
  ...
  private saveTrustedHost(host: string, publicKey: string): void {
    try {
      ...
      if (existing) {
        this.database
          .prepare(
            "UPDATE trusted_ssh_hosts SET key_type = ?, public_key = ?, updated_at = ? WHERE host = ?",
          )
        ...
        this.database
          .prepare(
            "INSERT INTO trusted_ssh_hosts (host, key_type, public_key, created_at, updated_at) VALUES (?, ?, ?, ?, ?)",
          )
