...
class OpenCodeServerManager {
  ...
  async start(): Promise<void> {
    ...
    const sshCredentials = gitCredentials.filter(
    ...
    if (this.version) {
      ...
      if (!this.isVersionSupported()) {
        logger.warn(
          `OpenCode version ${this.version} is below minimum required version ${MIN_OPENCODE_VERSION}`,
        );
  ...
  async reloadConfig(): Promise<void> {
    ...
    try {
      const response = await fetch(
        `http://${OPENCODE_SERVER_HOST}:${OPENCODE_SERVER_PORT}/config`,
        {
          method: "GET",
        },
      );
