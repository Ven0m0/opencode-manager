    ...
    return new Promise((resolve) => {
      ...
      proc.on("close", (code) => {
        if (code === 0 && stdout.trim()) {
          ...
          logger.debug(`Not a git repository: ${startPath}`, {
            stderr: stderr.trim(),
          });
          resolve(null);
        }
  ...
  try {
    ...
    const relativePaths = paths.map((p) => {
      ...
      return targetRelativeToRoot
        ? path.join(targetRelativeToRoot, relativeToTarget)
        : relativeToTarget;
    });
    ...
    return new Promise((resolve) => {
      ...
      proc.on("close", (code) => {
        ...
        logger.debug(
          ...
          ignoredFullPaths.length,
          "first 5:",
          ignoredFullPaths.slice(0, 5),
        );
        const targetRelativeToRoot = path.relative(gitRoot, targetPath);
...
async function filterIgnoredPaths(
  targetPath: string,
  allPaths: string[],
  options?: ArchiveOptions,
): Promise<string[]> {
...
export async function createRepoArchive(
  repoPath: string,
  options?: ArchiveOptions,
): Promise<string> {
...
export async function getIgnoredPathsList(
  ...
  if (!gitRoot) {
    ...
    const hasGitDir = await collectFiles(directoryPath)
      .then((paths) => paths.some((p) => p.startsWith(".git/") || p === ".git"))
      .catch(() => false);
  ...
  const gitDirExists = await stat(path.join(directoryPath, ".git"))
    .then(() => true)
    .catch(() => false);
