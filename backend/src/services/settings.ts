...
export class SettingsService {
  ...
  persistLastKnownGoodConfig(userId: string = "default"): void {
    if (SettingsService.lastKnownGoodConfigContent) {
      this.updateSettings(
        { lastKnownGoodConfig: SettingsService.lastKnownGoodConfigContent },
        userId,
      );
  ...
  createOpenCodeConfig(
    ...
    const result = this.db
      ...
      .run(userId, request.name, rawContent, shouldBeDefault, now, now);
  ...
  ensureSingleConfigIsDefault(userId: string = "default"): void {
    const hasDefault = this.db
      .query(
        "SELECT COUNT(*) as count FROM opencode_configs WHERE user_id = ? AND is_default = TRUE",
      )
      .get(userId) as { count: number };
    ...
    if (hasDefault.count === 0) {
      const firstConfig = this.db
        .query(
          "SELECT config_name FROM opencode_configs WHERE user_id = ? ORDER BY created_at ASC LIMIT 1",
        )
  ...
  restoreToLastKnownGoodConfig(
    userId: string = "default",
  ): { configName: string; content: string } | null {
    if (!SettingsService.lastKnownGoodConfigContent) {
