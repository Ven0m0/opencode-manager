...
import type { CreateRepoInput, Repo } from "../types/repo";
...
function enhanceCloneError(
  ...
  if (
    ...
    return new Error(
      `Authentication failed for ${repoUrl}. Please add your credentials in Settings > Git Credentials.`,
    );
  }

  if (message.includes("repository not found") || message.includes("404")) {
    return new Error(
      `Repository not found: ${repoUrl}. Check the URL and ensure you have access to it.`,
    );
  }

  if (isSSHUrl(repoUrl) && message.includes("permission denied")) {
    return new Error(
      `Access denied to ${repoUrl}. Please add your SSH credentials in Settings > Git Credentials and ensure your SSH key has access to this repository.`,
    );
  }

  if (
    isGitHubHttpsUrl(repoUrl) &&
    (message.includes("permission denied") || message.includes("fatal"))
  ) {
    return new Error(
      `Access denied to ${repoUrl}. Please add your credentials in Settings > Git Credentials and ensure you have proper access.`,
    );
  }

  if (message.includes("timed out")) {
    return new Error(
      `Clone timed out for ${repoUrl}. The repository might be too large or there could be network issues. Try again or verify the repository exists.`,
    );
  }
...
async function isValidGitRepo(
  ...
  try {
    await executeCommand(["git", "-C", repoPath, "rev-parse", "--git-dir"], {
      env,
      silent: true,
    });
    return true;
...
async function copyRepoToWorkspace(
  sourcePath: string,
  targetName: string,
  env: Record<string, string>,
): Promise<void> {
  ...
  await executeCommand(["git", "clone", "--local", sourcePath, targetName], {
    cwd: reposPath,
    env,
  });
...
async function safeGetCurrentBranch(
  repoPath: string,
  env: Record<string, string>,
): Promise<string | null> {
  try {
    ...
    if (!repoHasCommits) {
      try {
        const symbolicRef = await executeCommand(
...
async function checkoutBranchSafely(
  repoPath: string,
  branch: string,
  env: Record<string, string>,
): Promise<void> {
  ...
  try {
    await executeCommand(
      [
        ...
        "rev-parse",
        "--verify",
        `refs/remotes/origin/${sanitizedBranch}`,
      ],
      { env, silent: true },
    );
  ...
  if (localBranchExists) {
    ...
    await executeCommand(
      ["git", "-C", repoPath, "checkout", "-b", sanitizedBranch],
      { env },
    );
  }
}
...
export async function initLocalRepo(
  ...
  if (path.isAbsolute(normalizedInputPath)) {
    ...
    try {
      const exists = await executeCommand(["test", "-d", normalizedInputPath], {
        silent: true,
      })
        .then(() => true)
        .catch(() => false);
      ...
      if (isGit) {
        ...
        if (!isAvailable) {
          throw new Error(
            `A repository named '${baseName}' already exists in the workspace. Please remove it first or use a different source directory.`,
          );
        }
        ...
        logger.info(
          `Copying existing git repo from ${normalizedInputPath} to workspace as ${baseName}`,
        );
        await copyRepoToWorkspace(normalizedInputPath, baseName, env);
        targetPath = path.join(getReposPath(), baseName);
      } else {
        throw new Error(
          `Directory exists but is not a valid Git repository. Please provide either a Git repository path or a simple directory name to create a new empty repository.`,
        );
      }
    } catch (error: unknown) {
      if (
        isErrorWithMessage(error) &&
        getErrorMessage(error).includes("No such file or directory")
      ) {
        throw error;
      }
      throw new Error(
        `Failed to process absolute path '${normalizedInputPath}': ${getErrorMessage(error)}`,
      );
    }
  } else {
  ...
  try {
    ...
    logger.info(
      `Created database record for local repo: ${repoLocalPath} (id: ${repo.id})`,
    );
  } catch (error: unknown) {
    logger.error(
      `Failed to create database record for local repo: ${repoLocalPath}`,
      error,
    );
    throw new Error(
      `Failed to register local repository '${repoLocalPath}': ${getErrorMessage(error)}`,
    );
  }
  ...
  try {
    ...
    const isGitRepo = await executeCommand([
      "git",
      "-C",
      targetPath,
      "rev-parse",
    ...
    if (!isGitRepo) {
      throw new Error(
        `Git initialization failed - directory exists but is not a valid git repository`,
      );
    }
    ...
    logger.error(
      `Failed to initialize local repo, rolling back: ${repoLocalPath}`,
      error,
    );
    ...
    try {
      ...
      logger.error(
        `Failed to rollback database record for repo id ${repo.id}:`,
    ...
    if (directoryCreated && !sourceWasGitRepo) {
      try {
        ...
        logger.error(
          `Failed to rollback directory ${repoLocalPath}:`,
          getErrorMessage(fsError),
        );
      }
      ...
      try {
        ...
        logger.error(
          `Failed to clean up copied directory ${repoLocalPath}:`,
          getErrorMessage(fsError),
        );
      }
    }
    ...
    throw new Error(
      `Failed to initialize local repository '${repoLocalPath}': ${getErrorMessage(error)}`,
    );
  }
}
...
export async function cloneRepo(
  ...
  const hasSSHCredential = await gitAuthService.setupSSHForRepoUrl(
    repoUrl,
    database,
  );

  const { url: normalizedRepoUrl, name: repoName } = normalizeRepoUrl(
    repoUrl,
    preserveSSH,
  );
  ...
  const existing = db.getRepoByUrlAndBranch(
    database,
    normalizedRepoUrl,
    branch,
  );
  ...
  if (existing) {
    logger.info(
      `Repo branch already exists: ${normalizedRepoUrl}${branch ? `#${branch}` : ""}`,
    );
    if (hasSSHCredential) {
  ...
  const baseRepoExists = await executeCommand(
    ["bash", "-c", `test -d ${baseRepoDirName} && echo exists || echo missing`],
    path.resolve(getReposPath()),
  );

  const shouldUseWorktree =
    useWorktree && branch && baseRepoExists.trim() === "exists";
  ...
  try {
    ...
    if (shouldUseWorktree) {
      ...
      const worktreeVerified = await executeCommand([
        "test",
        "-d",
        worktreePath,
      ])
      ...
      logger.info(
        `Base repo exists but worktree creation failed, cloning branch separately`,
      );

      const worktreeExists = await executeCommand(
        [
          "bash",
          "-c",
          `test -d ${worktreeDirName} && echo exists || echo missing`,
        ],
        path.resolve(getReposPath()),
      );
      if (worktreeExists.trim() === "exists") {
        logger.info(
          `Workspace directory exists, removing it: ${worktreeDirName}`,
        );
        try {
          await executeCommand(["rm", "-rf", worktreeDirName], getReposPath());
          const verifyRemoved = await executeCommand(
            [
              "bash",
              "-c",
              `test -d ${worktreeDirName} && echo exists || echo removed`,
            ],
            getReposPath(),
          );
          if (verifyRemoved.trim() === "exists") {
            throw new Error(
              `Failed to remove existing directory: ${worktreeDirName}`,
            );
          }
        } catch (cleanupError: unknown) {
          logger.error(
            `Failed to clean up existing directory: ${worktreeDirName}`,
            cleanupError,
          );
          throw new Error(
            `Cannot clone: directory ${worktreeDirName} exists and could not be removed`,
          );
        }
      }
      ...
      try {
        await executeCommand(
          ...
          { cwd: getReposPath(), env, timeout: GIT_CLONE_TIMEOUT },
        );
      } catch (error: unknown) {
        if (
          isErrorWithMessage(error) &&
          getErrorMessage(error).includes("destination path") &&
          getErrorMessage(error).includes("already exists")
        ) {
          logger.error(
            `Clone failed: directory still exists after cleanup attempt`,
          );
          throw new Error(
            `Workspace directory ${worktreeDirName} already exists. Please delete it manually or contact support.`,
          );
        }
        ...
        if (
          branch &&
          isErrorWithMessage(error) &&
          (getErrorMessage(error).includes("Remote branch") ||
            getErrorMessage(error).includes("not found"))
        ) {
          logger.info(
            `Branch '${branch}' not found, cloning default branch and creating branch locally`,
          );
          try {
            await executeCommand(
            ...
            throw enhanceCloneError(
              cloneError,
              normalizedRepoUrl,
              getErrorMessage(cloneError),
            );
          }
          ...
          try {
            await executeCommand([
              "git",
              "-C",
              path.resolve(getReposPath(), worktreeDirName),
              "rev-parse",
              "--verify",
              `refs/heads/${branch}`,
            ]);
      ...
      if (baseRepoExists.trim() === "exists") {
        logger.info(
          `Repository directory already exists, verifying it's a valid git repo: ${baseRepoDirName}`,
        );
        const isValidRepo = await executeCommand(
          [
            "git",
            "-C",
            path.resolve(getReposPath(), baseRepoDirName),
            "rev-parse",
            "--git-dir",
          ],
          path.resolve(getReposPath()),
        ...
        if (isValidRepo.trim() === "valid") {
          ...
          logger.warn(
            `Invalid repository directory found, removing and recloning: ${baseRepoDirName}`,
          );
      ...
      logger.info(
        `Cloning repo: ${normalizedRepoUrl}${branch ? ` to branch ${branch}` : ""}`,
      );
    ...
    db.updateRepoStatus(database, repo.id, "ready");
    logger.info(
      `Repo ready: ${normalizedRepoUrl}${branch ? `#${branch}` : ""}${shouldUseWorktree ? " (worktree)" : ""}`,
    );
    return { ...repo, cloneStatus: "ready" };
  } catch (error: unknown) {
    logger.error(
      `Failed to create repo: ${normalizedRepoUrl}${branch ? `#${branch}` : ""}`,
      error,
    );
    db.deleteRepo(database, repo.id);
    throw error;
...
export async function createBranch(
  ...
  try {
    ...
    logger.info(
      `Successfully created and switched to branch: ${sanitizedBranch}`,
    );
...
export async function pullRepo(
  ...
  try {
    ...
    await executeCommand(
      ["git", "-C", path.resolve(getReposPath(), repo.localPath), "pull"],
      { env },
    );
...
function normalizeRepoUrl(
  ...
  if (url.startsWith("ssh://")) {
    ...
    return {
      url: preserveSSH
        ? url
        : `https://${host}/${pathPart.replace(/\.git$/, "")}`,
      name: repoName,
    };
  }
...
export async function cleanupOrphanedDirectories(
  database: Database,
): Promise<void> {
  try {
    const reposPath = getReposPath();
    ...
    const trackedPaths = new Set(
      allRepos.map((r) => r.localPath.split("/").pop()),
    );
...
async function createWorktreeSafely(
  baseRepoPath: string,
  worktreePath: string,
  branch: string,
  env: Record<string, string>,
  ...
  if (currentBranch === branch) {
    const defaultBranch = await executeCommand(
      ["git", "-C", baseRepoPath, "rev-parse", "--abbrev-ref", "origin/HEAD"],
      { env },
    )
    ...
    await executeCommand(
      ["git", "-C", baseRepoPath, "checkout", defaultBranch],
      { env },
    ).catch(() =>
      executeCommand(["git", "-C", baseRepoPath, "checkout", "main"], { env }),
    );
  }
  ...
  if (branchExists) {
    await executeCommand(
      ["git", "-C", baseRepoPath, "worktree", "add", worktreePath, branch],
      { env },
    );
  } else {
    await executeCommand(
