...
export class GitAuthService {
  ...
  initialize(ipcServer: IPCServer | undefined, database: Database): void {
    ...
    if (ipcServer) {
      ...
      logger.info(
        `SSH host key handler registered with IPC server at /${handlerPath}`,
      );
    }
  }
  ...
  async setupSSHKey(credential: GitCredential): Promise<void> {
    ...
    try {
      ...
      if (credential.passphrase) {
        ...
        logger.info(
          `SSH key with passphrase created for ${credential.name}: ${this.sshKeyPath}`,
        );
      } else {
        this.sshPassphrase = null;
        logger.info(
          `SSH key created for ${credential.name}: ${this.sshKeyPath}`,
        );
      }
    } catch (error) {
  ...
  getSSHEnvironment(): Record<string, string> {
    ...
    if (!this.sshKeyPath) {
      if (knownHostsPath) {
        ...
        logger.info(
          `SSH environment: Using known_hosts=${knownHostsPath}, SSH_COMMAND=${sshCommand}`,
        );
        return {
      ...
      logger.info(
        `SSH environment: No SSH key and no known_hosts path, returning host key handler env only`,
      );
      return {
        ...(this.sshHostKeyHandler?.getEnv() || {}),
      };
    ...
    const sshResult = buildSSHCommand(
      this.sshKeyPath,
      this.sshPassphrase || undefined,
      knownHostsPath,
      port,
    );
    logger.info(
      `SSH environment: Using SSH key=${this.sshKeyPath}, known_hosts=${knownHostsPath}, SSH_COMMAND=${sshResult.command}`,
    );
    return {
